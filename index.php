...
if ($_COOKIE['Any-Proxy'] != $pass) {
	header('HTTP/1.1 403');
    exit('<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport"content="width=device-width, initial-scale=1.0"><link rel="stylesheet" type="text/css" href="style.css"><style>form{width:90%;margin:0 auto;text-align:center}</style></head><body><div id="app" class="main"><div class="row row-xs"><div class="col-lg-6 col-md-6 col-sm-6 col-xs-10 col-xs-offset-1 col-sm-offset-3 col-md-offset-3 col-lg-offset-3"><div class="page-header"><h3 class="text-center h3-xs">心之所向 素履以往</h3></div><form method="post"><div class="search">

<input type="password" name="Any-Proxy" id="inputContent" autocomplete="on" placeholder="输入密码.." value="">

<button type="submit" onclick="search(); return false;">GO</button>

</div></form></div></div><body style="overflow:-Scroll;overflow-y:hidden" ></body></html>');
}
...
if (substr($path, 1, 7) == "http://" || substr($path, 1, 8) == "https://" || $_POST['urlss']) {
    ...
    exit('<html><head><meta charset="utf-8"><meta name="viewport" content="width=520, user-scalable=no, target-densitydpi=device-dpi"><title>心之所向 素履以往 富强、民主、文明、和谐、自由、平等、公正</title><link rel="stylesheet" type="text/css" href="style.css"><style type="text/css">.row{margin-top:100px}.page-header{margin-bottom:90px}.expand-transition{margin-top:150px;-webkit-transition:all.5s ease;transition:all.5s ease}</style></head><body><div id="app" class="main"><div class="row row-xs"><div class="col-lg-6 col-md-6 col-sm-6 col-xs-10 col-xs-offset-1 col-sm-offset-3 col-md-offset-3 col-lg-offset-3"><div class="page-header"><h3 class="text-center h3-xs">心之所向 素履以往</h3></div><form method="post"><div class="search">

<input type="search" name="urlss" id="inputContent" autocomplete="on" placeholder="请输入域名.." value="">
